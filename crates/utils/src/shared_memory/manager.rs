...
impl ThreadSafeSharedMemoryManager {
    ...
    pub fn create_shared_memory(&self, path: impl AsRef<Path>, configs: &[DeviceConfig]) -> Result<()> {
        ...
        F: FnOnce(&SharedMemoryHandle) -> Result<T>,
    {
        ...
        let shmem = memories
            .get(path.as_ref())
            .context(format!("Shared memory not found: {}", path.as_ref().display()))?;
        f(shmem)
    }
    ...
    fn remove_orphaned_file(&self, file_path: &std::path::Path,  path: impl AsRef<Path>) -> Result<()> {
...
impl super::traits::SharedMemoryAccess for ThreadSafeSharedMemoryManager {
    ...
    fn create_shared_memory(
        ...
        self.create_shared_memory(std::path::Path::new(pod_identifier), cfgs)
    }
    ...
    fn get_shared_memory(
        ...
        self.get_shared_memory(std::path::Path::new(pod_identifier))
            .map(|ptr| ptr as *const _)
    }

    fn add_pid(&self, pod_identifier: &str, host_pid: usize) -> Result<(), Self::Error> {
        self.add_pid(std::path::Path::new(pod_identifier), host_pid)
    }

    fn remove_pid(&self, pod_identifier: &str, host_pid: usize) -> Result<(), Self::Error> {
        self.remove_pid(std::path::Path::new(pod_identifier), host_pid)
    }
