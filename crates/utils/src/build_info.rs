...
pub static BUILD_INFO: LazyLock<BuildInfo> = LazyLock::new(BuildInfo::from_build);
...
impl fmt::Display for BuildInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "Commit SHA:\t\t{:?}{}",
            self.commit_sha1.unwrap_or("unknown"),
            if self.git_dirty { "-dirty" } else { "" }
        )?;
        write!(f, "Package version:\t{}", env!("CARGO_PKG_VERSION"))?;
        Ok(())
    }
}
...
fn prerelease_version(describe: &'static str) -> Option<DescribeResult> {
    ...
    Some(DescribeResult::Prerelease {
        version: describe,
        major,
        minor,
        patch,
...
mod test {
    ...
    fn assert_not_a_tag(describe: &'static str) {
        assert_eq!(
            DescribeResult::NotATag { describe },
    ...
    fn assert_proto(describe: &'static str) {
        assert_eq!(
            DescribeResult::Prototype { name: describe },
            DescribeResult::new(describe)
        )
    }
